...
st.subheader("☠️ 두피진단과 두피케어 제품 추천 서비스")
# st.markdown("""
#     <style>
#     .lefted-title {
#         text-align: left;
...
#     }
#     </style>
#     <h1 class="lefted-title">☠️ 두피진단과 두피케어 제품 추천 서비스</h1>
#     """, unsafe_allow_html=True)
...
#     "건성" :
# """
# 건성 타입은 이틀정도 머리를 감지 않아도 기름기가 끼지 않고  머리감은 후의 청결상태가 유지된다면 건성타입의 두피입니다.
# 날씨가 건조해짐에 따라 정전기와 함께 두피가 당기는 듯한 건조함이 느껴지기도 합니다.
# 샴푸는 매일 하실 것을 권장하나, 심한 건성두피의 경우 이틀에 한번 샴푸하시는게 적당합니다.
#
# - 이틀쯤 머리를 감지않아도 기름이 끼지 않는다.
# - 모발이 푸석 거리고 윤기가 없다.
# - 모발에 정전기가 난다.
# - 바람이 많이 불때는 따갑거나 두피가 당기는 느낌이들 정도로 예민하다.
# - 외부자극(햇빛/펌/염색 등)에 쉽게 손상되었다.
# """,
#     "지성" :
# """
# 지성 타입은 머리를 감은 후 3~4시간정도 경과 후  모발이 가라 않으면서 기름이 끼고 냄새가 나며  하루 두번이상 샴푸를 해 주어야 청결이 유지되는 편이라면 지성 두피입니다.
# 보통 피부가 지성이신 분들의 경우 두피도 지성타입에 속한 경우가 많습니다.
# - 머리를 감은지 몇시간이 지나지 않아도 기름기가 생긴다.
# - 피지로 냄새가 나는 두피
# - 비듬이나 각질 등으로 지속적인 관리가 필요한 두피
# - 모발이 힘이 없고 쳐저서 볼륨이 약하다.
# - 자주 가려운 증상이 있다.
# """,
#     "지루성" :
# """
# 지루성 타입은 온도와 습도 등 계절적인 변화, 그 외 박테리아와 효모균으로 인해 염증이 생기는 피부질환으로 지루성 두피염을 가진 사람은 두피 통증, 각질, 비듬, 두피 건조증, 뾰루지 등의 증상이 지속되는 특징이 있습니다.
# """
# }
...
# 탭 생성
tab1, tab2, tab3, tab4, tab5 = st.tabs(["[홈페이지]", "[설문조사]", "[두피진단]", "[증상설명]", "[제품추천]"])

with tab1:
    ############################ 0. 소개와 안내사항을 알려줍니다. ############################
    st.markdown("**🔥 서비스 소개와 안내 사항**")
    st.image("./data/introduction1.png", use_column_width=True)
    st.image("./data/introduction2.png", use_column_width=True)
    st.image("./data/introduction3.png", use_column_width=True)
...
# }
# </style>
# <h1 class="lefted-body">
#
# 1. 소개 및 이용방법
#
# 딥러닝을 통한 두피 진단과 두피케어 제품 추천 서비스
#
# [설문조사] - [두피진단] - [증상설명] - [제품추천] 순으로 진행합니다.
#
# [설문조사]
# - 이름, 성별, 나이, 고민 증상, 사진 위치, 샴푸 사용 빈도, 펌주기, 염색주기, 현재 모발 상태, 사용 제품, 맞춤 두피 케어 제품 사용 유무, 추천 제품, 샴푸 구매시 고려 부분과 같은 정보를 입력합니다.
# - 입력한 사용자의 두피관리 방법과 메타데이터를 통해 미리 두피 상태를 진단해 봅니다.
#
# [두피진단]
# - 고민이 되는 증상이 있는 두피 위치에 예제과 같이 두피 사진을 찍어 컴퓨터에 미리 저장해 놓습니다.
# - 위치는 정수리 : TH, 좌측두 : LH, 우측두 : RH, 후두부 : BH 입니다.
# - 컴퓨터에 저장해 놓은 위치로 이동해 파일을 선택합니다.
# - 각 증상은 비듬, 미세각질, 미세사이홍반, 미세홍반농포, 피지과다, 탈모입니다.
# - 딥러닝으로 학습된 모델을 통해 각 증상별로 양호, 경증, 중증으로 나누어 판단해 줍니다.
# - 딥러닝으로 학습된 모델이 가장 높은 확률로 판단할 때
#
# [증상설명]
# - 쳇 GPT로 진단 결과(경증, 중증)에 따른 각 원인, 특징 그리고 관리방안을 알려줍니다.
#
# [제품추천]
# - 진단 결과(경증, 중증)와 제품의 성분의 매칭을 통해 평이 좋고 리뷰가 많은 제품을 원하는 종류 별로 각각 3개씩 추천해 줍니다.
# - 원하는 종류는 설문조사에서 선택했으며 그 종류는 샴푸, 린스/컨디셔너, 샴푸바/드라이샴푸, 헤어오일/헤어세럼, 헤어워터, 두피책/스케일러, 헤어토닉/두피토닉이 있습니다.
#
# 2. 안내사항
# 1) 반드시 순서를 지켜주세요.
# 2) 이름과 같은 개인 정보는 시스템에 저장되지 않습니다.
#
# </h1>
# """, unsafe_allow_html=True)
    ...
    with st.form(key="form"):
        ...
        if submit:
            if not name:
                ...
                # st.text(f"{name}님 환영합니다!!!")
                # st.text(f"{symptom} 증상이 있는 부분을 예제 이미지와 유사하게 찍어서 올려주세요!")
                # st.text(f"그러면 {name}님에게 {variety}를 추천해드리겠습니다.")
    if submit:
        # symptom = ",".join(st.session_state.scalp[0]["symptom"])
        # variety = ",".join(st.session_state.scalp[0]["variety"])
        # st.markdown(f"""
        ...
        #     ]
        #     </style>
        #     <li class="centered-title">[<b>{st.session_state.scalp[0]["name"]}</b>]님 환영합니다!!!✨</li>
        #     <li class="centered-title">[🤖 <b>{symptom}</b>] 증상이 있는 부분을 [<b>⬇ 예제 이미지</b>]와 유사하게 찍어서 올려주세요!</li>
        #     <li class="centered-title">이미지 진단의 결과에 따른 [<b>두피 상태의 원인과 특징 그리고 관리방안</b>🏸]을 알려 드리고,</li>
        #     <li class="centered-title">[🔮 <b>{variety}</b>](을)를 추천해드리겠습니다.</li>
        #     """, unsafe_allow_html=True
        #             )
        # st.write("")
        # st.write("")
...
with tab3:
    ...
    if name == "":
        st.write("")
        st.text("* 설문조사를 하고 오세요!")
        st.text("* 분석을 위해 설문조사 결과와 두피 진단 결과가 동시에 시스템에 저장됩니다.")
    else:
        st.markdown("**🔥 예제 두피 이미지**")
        st.text("* 아래 두피 이미지와 유사하게 사진을 찍어 올려주세요!")
        ...
        if uploaded_file is not None:
            ...
            st.markdown("**🔥 사용자 두피 이미지 결과**")
            st.text("* 두피의 상태는 증상에 따라, 양호, 경증, 중증으로만 분류하여 줍니다.")
            cols = st.columns(2)
            with cols[0]:
                with st.expander(label="※ 사용자의 이미지", expanded=True):
                    ...
                    st.write("")
            with cols[1]:
                with st.expander(label="※ 사용자의 두피 상태 결과", expanded=True):
...
with tab4:
    ############################ 5. 두피 타입별 원인과 특징 그리고 관리방안을 보여주기 ############################
    ...
    if bidum_state == "" and gakzil_state == "" and hongban_state == "" and nongpo_state == "" and pizy_state == "" and talmo_state == "":
        st.write("")
        st.text("* 설문조사와 두피진단을 하고 오세요!")
        st.text("* 설문조사와 두피진단 결과에 따라 두피 타입 별 원인과 특징 그리고 관리방안을 설명해 드리겠습니다.")
    else:
        st.write("")
...
with tab5:
    ...
    if bidum_state == "" and gakzil_state == "" and hongban_state == "" and nongpo_state == "" and pizy_state == "" and talmo_state == "":
        st.write("")
        st.text("* 설문조사와 두피진단을 하고 오세요!")
        st.text("* 설문조사와 두피진단 결과에 따라 제품을 추천해 드리겠습니다.")
    else:
        st.write("")
